...
def filter_psr(psr, bw=1.1, dt=7, filter_dict=None, min_toas=10,
    ...
    idx_flag = np.flatnonzero(np.all(flag_keep, axis=0))
    ...
    if not sum(mask):
        ...
        # This results in blank/not useful entries in FoM dictionary 
        return psr
    ...
    if np.sum(~mask) == psr.nobs:
        ...
        if plot:
            ...
            for pta in np.unique(psr.flagvals('pta')):
                ...
                plt.plot(psr.toas()[nix], psr.freqs[nix], '.', label=pta, alpha=0.2)
            plt.plot(psr.toas()[~psr.deletedmask()], psr.freqs[~psr.deletedmask()], '.',
                     color='C3', alpha=0.5, label='filtered')
